...
def mod():
    ...
    lblInfo=Label(root,font=('tohoma',30,'bold'),text="Modify Student Records",fg="Black",bd=10,anchor='w')
    lblInfo.pack()
    ...
    def modi():
        ...
        z=m.get()
        n=r.get()
        query="UPDATE RECORDS SET {}='{}'WHERE ROLLNO={}".format(k,z,n)
        d.execute(query)
        mydb.commit()
    ...
    m=StringVar()
    r=IntVar()
    input_c=Entry(root,width=5,bd=8,textvariable=c)
    input_c.place(x=220,y=150)
    input_m=Entry(root,width=5,bd=8,textvariable=m)
    input_m.place(x=220,y=200)
    input_r=Entry(root,width=5,bd=8,textvariable=r)
    input_r.place(x=220,y=250)

    btn=Button(root,text='Modify',padx=16,bd=8,fg='black',font=('Arial',12,'bold'),command=modi)
